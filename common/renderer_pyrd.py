...
class Renderer(object):
    ...
    def __init__(self, focal_length=600, img_w=512, img_h=512, faces=None, camera_center=None,
    ...
    def render_front_view(self, verts, bg_img_rgb=None, bg_color=(0, 0, 0, 0)):
        """
        verts: (num_persons, 6890, 3)
        bg_img_rgb (Optional): (img_h, img_w, 3)

        returns: image rgb (img_h, img_w, 3)
        """
        # Create a scene for each image and render all meshes
        scene = pyrender.Scene(bg_color=bg_color, ambient_light=np.ones(3) * 0)
        # Create camera. Camera will always be at [0,0,0]

        camera = pyrender.camera.IntrinsicsCamera(fx=self.fx, fy=self.fy,
        ...
        for n in range(num_people):
            ...
            if self.same_mesh_color:
                mesh_color = colorsys.hsv_to_rgb(0.6, 0.5, 1.0)  # 绘制人体的颜色
            else:
        ...
        color_rgb = color_rgba[:, :, :3]  # (img_h, img_w, 3)
        if bg_img_rgb is None:
            return color_rgb
        else:
            assert bg_img_rgb.shape == color_rgb.shape, "bg_img_rgb.shape != color_rgb.shape"
            mask = depth_map > 0
