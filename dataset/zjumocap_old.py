...
class ZJUMoCapDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        self.subject: str = cfg.subject  # "CoreView_377"
        self.train_frames = cfg.train_frames  # [0, 570, 1]
        self.train_cams = cfg.train_views  # ['1', '2']
        ...
        self.faces: np.ndarray = np.load('body_models/misc/faces.npz')['faces']  # (n, 3)
        self.skinning_weights: Dict[str, np.ndarray] = dict(np.load('body_models/misc/skinning_weights_all.npz'))
        self.posedirs: Dict[str, np.ndarray] = dict(np.load('body_models/misc/posedirs_all.npz'))
        self.J_regressor: Dict[str, np.ndarray] = dict(np.load('body_models/misc/J_regressors.npz'))
        if split == 'train':
            cam_names: List[str] = self.train_cams
            frames: List[int] = self.train_frames
        elif split == 'val':
        ...
        camera_parameters_path = os.path.join(self.root_dir, self.subject, 'cam_params.json')
        with open(camera_parameters_path, 'r') as f:
            self.cameras: Dict[str, dict] = json.load(f)
        ...
        start_frame, end_frame, sampling_rate = frames  # [0, 570, 1]
        ...
        if split == 'predict':
            ...
            # select the action sequence for prediction
